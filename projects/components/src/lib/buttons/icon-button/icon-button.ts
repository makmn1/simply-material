...
export class IconButton implements AfterViewInit {
  ...
  private pressedMorphVar() { return `--md-comp-icon-button-${this.size()}-shape-pressed-morph`; }
  private restingShapeVar(shape: IconButtonShape | undefined = undefined) {
    if (!shape) shape = this.shape();
    return `--md-comp-icon-button-${this.size()}-container-shape-${shape}`;
  }
  private selectedShapeVar(shape: IconButtonShape | undefined = undefined) {
    if (!shape) shape = this.shape();
    return `--md-comp-icon-button-${this.size()}-selected-container-shape-${shape}`;
  }
  private springDampingVar() { return `--md-comp-icon-button-${this.size()}-shape-spring-animation-damping`; }
  private springStiffnessVar() { return `--md-comp-icon-button-${this.size()}-shape-spring-animation-stiffness`; }
  ...
  private readonly _shapeSync = resource({
    ...
    loader: async ({params: newShape}) => {
      if (this._viewInitialized) {
        const el = this.root();
        const oldShape: IconButtonShape = this.shape() == "round" ? "square" : "round";
  ...
  private getCurrentBorderRadius(selected: boolean, shape: IconButtonShape | undefined = undefined): string {
