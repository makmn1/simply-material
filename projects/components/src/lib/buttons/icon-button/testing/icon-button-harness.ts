...
/** Harness for interacting with a sm-icon-button in tests. */
export class SmIconButtonHarness extends ContentContainerComponentHarness {
  static hostSelector = 'button[sm-icon-button], a[sm-icon-button]';
   ...
   *   - `toggle` finds icon buttons matching a specific toggle state.
   ...
   */
  static with<T extends SmIconButtonHarness>(
  ...
  async getWidth(): Promise<IconButtonWidth | null> {
    const host = await this.host();
    const width = await host.getAttribute('data-width');
  ...
  /** Gets whether the icon button is selected. Only applicable for toggle buttons. */
