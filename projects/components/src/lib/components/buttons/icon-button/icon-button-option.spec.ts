...
class IconButtonOptionPage extends IconButtonPage {
  override getComponent(testId: string): SimplyMatIconButtonOption | null {
    ...
    const allComponents = this.fixture.debugElement.queryAll(By.directive(SimplyMatIconButtonOption));
    const debugElement = allComponents.find((el) => el.nativeElement === element);
    if (!debugElement) return null;
    try {
      return debugElement.injector.get(SimplyMatIconButtonOption, null);
    } catch {
      return null;
    }
  }
...
@Component({
  template: `
    ...
    <button
      data-testid="outlined-option-btn"
      simplyMatIconButton
      ngOption
      [variant]="'outlined'">
      <span>Icon</span>
    ...
    <button
      data-testid="standard-option-btn"
      simplyMatIconButton
      ngOption
      [variant]="'standard'">
      <span>Icon</span>
  ...
  `,
  imports: [SimplyMatIconButtonOption],
})
class IconButtonOptionTestComponent {
...
describe('SimplyMatIconButtonOption', () => {
  let fixture: ComponentFixture<IconButtonOptionTestComponent>;
  let testPage: IconButtonOptionPage;
  let testComponent: IconButtonOptionTestComponent;

  beforeEach(async () => {
    TestBed.configureTestingModule({
      imports: [IconButtonOptionTestComponent, SimplyMatIconButtonOption],
    });

    fixture = TestBed.createComponent(IconButtonOptionTestComponent);
    testPage = new IconButtonOptionPage(fixture);
  ...
  describe('Class Inheritance', () => {
    test('should extend SimplyMatIconButton class', () => {
      const component = testPage.getComponent('default-option-btn');
      expect(component).toBeTruthy();
      expect(component).toBeInstanceOf(SimplyMatIconButtonOption);
    ...
    test('should provide BUTTON_BASE_CONFIG token', () => {
      ...
      const debugElement = fixture.debugElement.queryAll(By.directive(SimplyMatIconButtonOption))
  ...
  describe('Inherited Behavior - Variant', () => {
    ...
    test('should respect variant input - tonal', () => {
      const component = testPage.getComponent('tonal-option-btn');
      expect(component?.baseConfig.buttonVariant()).toBe('tonal');
      expect(testPage.getAttribute('tonal-option-btn', 'data-sm-variant')).toBe('tonal');
    });

    test('should respect variant input - outlined', () => {
      const component = testPage.getComponent('outlined-option-btn');
      expect(component?.baseConfig.buttonVariant()).toBe('outlined');
      expect(testPage.getAttribute('outlined-option-btn', 'data-sm-variant')).toBe('outlined');
    });

    test('should respect variant input - standard', () => {
      const component = testPage.getComponent('standard-option-btn');
      expect(component?.baseConfig.buttonVariant()).toBe('standard');
      expect(testPage.getAttribute('standard-option-btn', 'data-sm-variant')).toBe('standard');
    });
  ...
  describe('Inherited Behavior - Width', () => {
    test('should respect width input - default', () => {
      const component = testPage.getComponent('default-option-btn');
      expect(component?.width()).toBe('default');
      expect(testPage.getAttribute('default-option-btn', 'data-sm-width')).toBe('default');
    });

    test('should respect width input - narrow', () => {
      const component = testPage.getComponent('narrow-option-btn');
      expect(component?.width()).toBe('narrow');
      expect(testPage.getAttribute('narrow-option-btn', 'data-sm-width')).toBe('narrow');
    });

    test('should respect width input - wide', () => {
      const component = testPage.getComponent('wide-option-btn');
      expect(component?.width()).toBe('wide');
      expect(testPage.getAttribute('wide-option-btn', 'data-sm-width')).toBe('wide');
    });

    test('should update width reactively', async () => {
      const component = testPage.getComponent('dynamic-option-btn');
      expect(component?.width()).toBe('default');
      ...
      expect(component?.width()).toBe('wide');
      expect(testPage.getAttribute('dynamic-option-btn', 'data-sm-width')).toBe('wide');
    });
  });
  ...
  describe('Differences from SimplyMatIconButton', () => {
